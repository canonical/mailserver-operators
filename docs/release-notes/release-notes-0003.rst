...
Added support for LUKS-encrypted block storage for mail data. When a ``mail-data`` storage is attached and the ``luks-auto-provisioning`` configuration option is enabled, the charm automatically formats the device with LUKS encryption using the passphrase from the ``luks-key`` Juju secret, opens the device via ``cryptsetup``, creates an ext4 filesystem, and mounts it at ``/srv/mail``. Persistent mount configuration is written to ``/etc/fstab``. When ``luks-auto-provisioning`` is disabled, the charm monitors the mount point directly and sets the unit status accordingly. Storage detach events cleanly unmount the filesystem and close the LUKS device.
