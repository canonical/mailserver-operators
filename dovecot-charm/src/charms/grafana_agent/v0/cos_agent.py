...
class COSAgentRequirer(Object):
    ...
    def _on_relation_data_changed(self, event: RelationChangedEvent):
        ...
        if len(units) > 1:
            # should never happen
            raise ValueError(
                f"unexpected error: subordinate relation {cos_agent_relation} should have exactly one unit"
            )
        ...
        try:
            ...
            if isinstance(msg, bytes):
                if msg.startswith(
                    ...
                    logger.error(
                        f"encountered error {e} while attempting to update_relation_data.The relation must be gone."
                    )
                    return
            raise
